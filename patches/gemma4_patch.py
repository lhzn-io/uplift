                ...
                for (
                    ...
                    if weight_name in name:
                        if name.endswith(".weight_packed"):
                            moe_name = name.replace(weight_name + ".weight_packed", param_name.replace("_weight", "") + "_weight_packed")
                        elif name.endswith(".weight_scale"):
                            moe_name = name.replace(weight_name + ".weight_scale", param_name.replace("_weight", "") + "_weight_scale")
                        elif name.endswith(".weight_shape"):
                            moe_name = name.replace(weight_name + ".weight_shape", param_name.replace("_weight", "") + "_weight_shape")
                        else:
                    ...
                    # Quantized weights (like AWQ scales/shapes) may not be 2D
                    
                    weight_loader = param.weight_loader
                    
                    if name.endswith(".weight_packed"):
                        actual_weight_name = weight_name + ".weight_packed"
                    elif name.endswith(".weight_scale"):
                        actual_weight_name = weight_name + ".weight_scale"
                    elif name.endswith(".weight_shape"):
                        actual_weight_name = weight_name + ".weight_shape"
                    else:
                        actual_weight_name = weight_name + ".weight"
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def _weight_iterator():
            ...
            for name, weight in weights:
                ...
                name = re.sub(r"(\.layers\.\d+)\.experts\.", r"\1.moe.experts.", name)
                ...
                elif ".moe.experts.down_proj" in name:
                    name = name.replace(
